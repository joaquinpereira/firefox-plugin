...
// Source Code   - https://github.com/muaz-khan/browser-Extensions

// this page is using desktopCapture API to capture and share desktop
// http://developer.browser.com/extensions/desktopCapture.html

// browser.browserAction.onClicked.addListener(captureDesktop);
...
browser.runtime.onMessageExternal.addListener(
  function(request, sender, sendResponse) {    
    ...
    if (request.msjBunker){        
        
        if(request.msjBunker == 'take-picture'){
        ...
        if(request.msjBunker == 'stop-recording'){
        ...
        if(request.msjBunker == 'full-screen'){            
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'false',
                enableCamera: 'false',
        ...
        if(request.msjBunker == 'full-screen-audio'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'false',
                enableCamera: 'false',
        ...
        if(request.msjBunker == 'selected-tab'){
            browser.storage.sync.set({
        ...
        if(request.msjBunker == 'microphone-screen'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
        ...
        if(request.msjBunker == 'microphone-screen-camera'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'true', // TRUE
        ...
        if(request.msjBunker == 'microphone-webcam'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'true', // TRUE
                enableCamera: 'true', // TRUE
        ...
        if(request.msjBunker == 'see_all_recordings'){

            var url = 'browser-extension://' + browser.runtime.id + '/screen-recording/preview.html';    
            window.open(url,"_blank");            
        }

        if(request.msjBunker == 'btn-options'){

            var url = 'browser-extension://' + browser.runtime.id + '/screen-recording/options.html';    
            window.open(url,"_blank");    
        }

        if(request.msjBunker == 'stop-sharing'){
            browser.storage.sync.set({
        ...
        if(request.msjBunker == 'full-screen-p'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'false',
                enableCamera: 'false',
        ...
        if(request.msjBunker == 'full-screen-audio-p'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'false',
                enableCamera: 'false',
        ...
        if(request.msjBunker == 'full-screen-audio-microphone-p'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'true', // TRUE
                enableCamera: 'false',
        ...
        if(request.msjBunker == 'full-screen-audio-microphone-camera-p'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'true', // TRUE
                enableCamera: 'true',
        ...
        if(request.msjBunker == 'selected-tab-p'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'true', // TRUE
                enableMicrophone: 'false',
                enableCamera: 'false',
        ...
        if(request.msjBunker == 'microphone-screen-p'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'true', // TRUE
                enableCamera: 'false',
        ...
        if(request.msjBunker == 'microphone-screen-camera-p'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'true', // TRUE
                enableCamera: 'true', // TRUE
        ...
        if(request.msjBunker == 'microphone-webcam-p'){
            browser.storage.sync.set({
                enableTabCaptureAPI: 'false',
                enableMicrophone: 'true', // TRUE
                enableCamera: 'true', // TRUE
        ...
        if(request.msjBunker == 'btn-options-p'){
            var url = 'browser-extension://' + browser.runtime.id + '/desktopCapture-p2p/options.html';    
            window.open(url,"_blank"); 
        }
...
function handleWEBRTCEvents(message){
    console.log("message.startRecording: "+message.startRecording);
    console.log("message.stopRecording: "+message.stopRecording);
    console.log("message.startSharing: "+message.startSharing);
    console.log("message.stopSharing: "+message.stopSharing);
...
function captureDesktop() {
    ...
    browser.storage.sync.get(null, function(items) {
        ...
        if (_resolutions === '4K') {
            alert('"4K" resolutions is not stable in browser. Please try "fit-screen" instead.');
        }
        ...
        if (enableCamera || enableMicrophone) {
            captureCamera(function(stream) {
                ...
                desktop_id = browser.desktopCapture.chooseDesktopMedia(sources, function(browserMediaSourceId, opts) {
                    opts = opts || {};
                    opts.resolutions = resolutions;
                    opts.stream = stream;
                    onAccessApproved(browserMediaSourceId, opts);
                });
            });
            return;
        }
    ...
    browser.tabs.query({
        ...
        if (!!enableSpeakers) {
            ...
            constraints.audioConstraints = {
                ...
                optional: [{
                    googDisableLocalEcho: false // https://www.browserstatus.com/feature/5056629556903936
                }]
            };
        }
...
function onAccessApproved(browserMediaSourceId, opts) {
    if (!browserMediaSourceId) {
        setDefaults();
        return;
    }
...
function gotStream(stream) {
    ...
    // as it is reported that if you drag browser screen's status-bar
    // and scroll up/down the screen-viewer page.
    // browser auto-stops the screen without firing any 'onended' event.
    // browser also hides screen status bar.
    browser.windows.create({
        url: browser.extension.getURL('_generated_background_page.html'),
        type: 'popup',
        focused: false,
        width: 1,
        height: 1,
        ...
        setTimeout(function() {
            // browser.windows.remove(background_page_id);
        }, 3000);
    });
...
function setupRTCMultiConnection(stream) {
    ...
    function roomOpenCallback() {
        ...
        if(room_url_box === true) {
            ...
            var resultingURL = 'http://'+ip+'/desktop-capturing/index.php/?s=' + connection.sessionid;
            //var resultingURL = 'http://localhost:9001/?s=' + connection.sessionid;
...
function setDefaults() {
    ...
    if (popup_id) {
        try {
            browser.windows.remove(popup_id);
        } catch (e) {}
...
function captureCamera(callback) {
    ...
    navigator.mediaDevices.getUserMedia(constraints).then(function(stream) {
        ...
        browser.tabs.create({
            url: 'screen-recording/camera-mic.html'
        });
    });
}
